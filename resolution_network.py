...
from tf_slim.nets import resnet_v2, vgg
import tensorflow.compat.v1 as tf_c
tf_c.disable_v2_behavior()
...
class ResoNet(object):
    ...
    def build(self):
        # Input
        self.input = tf_c.placeholder(dtype=tf.float32, shape=[None, self.img_size[0], self.img_size[1], self.img_size[2]])
        ...
        if self.base_net == 'vgg16':
            ...
            with slim.arg_scope(resnet_v2.resnet_arg_scope()):
                net, end_points = resnet_v2.resnet_v2_50(self.input_mean, self.num_classes, is_training=self.is_train)
                self.prob = tf.nn.softmax(net[:, 0, 0, :], -1)
                self.logits = net[:, 0, 0, :]
        elif self.base_net == 'res101':
            with slim.arg_scope(resnet_v2.resnet_arg_scope()):
                net, end_points = resnet_v2.resnet_v2_101(self.input_mean, self.num_classes, is_training=self.is_train)
                self.prob = tf.nn.softmax(net[:, 0, 0, :], -1)
                self.logits = net[:, 0, 0, :]
        elif self.base_net == 'res152':
            with slim.arg_scope(resnet_v2.resnet_arg_scope()):
                net, end_points = resnet_v2.resnet_v2_152(self.input_mean, self.num_classes, is_training=self.is_train)
                self.prob = tf.nn.softmax(net[:, 0, 0, :], -1)
                self.logits = net[:, 0, 0, :]
        else:
            raise ValueError('base network should be vgg16, res50, -101, -152...')
        self.gt = tf_c.placeholder(dtype=tf.int32, shape=[None])
